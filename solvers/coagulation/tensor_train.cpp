...
void TTensorTrain::load_from_file(char *filename)
{
    ...
    free(ranks);
    ranks = NULL;
    size_t gg = fread(&this->dimensionality, sizeof(int), 1, f);
    ...
    carriages = (double **) malloc(this->dimensionality*sizeof(double*));
    gg=fread(this->modes_sizes, sizeof(int), this->dimensionality, f);
    gg = fread(this->ranks, sizeof(int), this->dimensionality + 1, f);
    for (int k = 0; k < this->dimensionality; k++)
    {
        carriages[k] = (double *) malloc(this->ranks[k] * this->modes_sizes[k] * this->ranks[k + 1] * sizeof(double));
        gg = fread(this->carriages[k], sizeof(double), this->ranks[k] * this->modes_sizes[k] * this->ranks[k + 1], f);
    }
    fclose(f);
}
